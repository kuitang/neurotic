function [ gmm ] = gmm_gibbs_diag_iter( gmm, X )
    ...
    % SPECIAL CASE THE BACKGROUND MODEL
    D2_log_2pi = D/2 * log(2*pi);
    sum_log_lam = 1/2 * sum(log(gmm.lam), 2);
    assert(all(size(sum_log_lam) == [gmm.K 1]));
    
    %Dxy_log_2pi = 2/2 * log(2*pi);
    %sum_log_lam_xy = 1/2 * sum(log(gmm.lam(1,1:2)));    
    ...
    for n = 1:N
        ...
        % Background model
        % Likelihood: product of just first two Gaussians + uniform
        %[spatial_like my_like]
        %assert(abs(spatial_like - my_like) < 1e-6);        
        % Background is white; use a stupid triangular p(x) = 2x
        x_like(1) = 1/N * (2 - 2 * X(n,3));
        
        for k = 2:gmm.K
            % Hotspot
            dx = X(n,:) - gmm.mu(k,:);
            quadform = -1/2 * sum(gmm.lam(k,:) .* (dx.^2));
